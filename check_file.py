    ...
    for early_stop_type in early_stop_type_list:
        for alpha in alpha_list:
            for threshold in threshold_list:
                ...
                cur_path = '/srv/scratch/z5102138/test_new_loss_v4_katana/test_{0}/result_{1}_{2}/'.format(dataset, train_method, test_method)                
                if os.path.exists(cur_path):
                    cur_command_list.append('mv /srv/scratch/z5102138/test_new_loss_v4_katana/test_{0}/result_{1}_{2}/ /srv/scratch/z5102138/test_new_loss_v4/test_{0}/\n'.format(dataset, train_method, test_method))
                        
                cur_path = '/srv/scratch/z5102138/test_new_loss_v4_katana/test_{0}/model_{1}/'.format(dataset, train_method)
                if os.path.exists(cur_path):
                    cur_command_list.append('mv /srv/scratch/z5102138/test_new_loss_v4_katana/test_{0}/model_{1}/  /srv/scratch/z5102138/test_new_loss_v4/test_{0}/\n'.format(dataset, train_method)) 
        
                if len(cur_command_list) != 0:
                    cur_command_list.append('\n\n\n')
                    command_list.append(cur_command_list)    

with open('mv_file.sh', 'w') as fsh:
    fsh.write('#!/bin/bash\n')
    fsh.write('set -e\n\n\n')      
